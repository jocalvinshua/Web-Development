...
/**
 * Add Product + Insert Images dengan is_main
 */
export const addProduct = async (productData, images = []) => {
  try {
    ...
    // 2. Insert ke tabel product_images dengan logika is_main
    if (images.length > 0) {
      const imageRows = images.map((url, index) => ({
        product_id: product.id,
        image_url: url,
        is_main: index === 0, // Gambar pertama (index 0) otomatis jadi gambar utama
      }));
...
/**
 * Fetch Admin (Mengambil data lengkap beserta gambar)
 */
...
/**
 * Fetch Client (Hanya yang tersedia + Gambar Utama)
 */
